...
Then, the second step of the `minimax` algorithm is to evaluate all the leaf nodes using an **utility function** that we have defined. The <a href="#figI35">Figure I.3.5</a> shows the same tree where the leafs have been evaluated by the same utility function.
...
we implement the algorithm in a computer language. The `alpha-beta pruning` (and the `minimax` algorithm) are `depth-first` search algorithms, which means that they expand the nodes from **top to bottom** and **left to right**. The <a href="#videoI31">Video I.3.1</a> shows how the tree is expanded by the `alpha-beta` algorithm.
...
understand how this AI works, so stay tuned!
